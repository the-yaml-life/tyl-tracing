use tyl_tracing::{TracingManager, SimpleTracer, TraceConfig, Environment};
...
fn custom_config_example() -> Result<(), tyl_tracing::TracingError> {
    ...
    println!("  - Completed spans: {}", tracer.get_completed_spans().len());
    println!();
    Ok(())
}
...
fn span_hierarchy_example() -> Result<(), tyl_tracing::TracingError> {
    ...
    tracer.add_span_attribute(&db_span_id, "query", serde_json::json!("SELECT * FROM users"))?;
    tracer.end_span(db_span_id)?;
    
    let validation_span_id = tracer.start_span("input_validation", Some(parent_span_id.clone()))?;
    tracer.add_span_attribute(&validation_span_id, "fields", serde_json::json!(["email", "name"]))?;
    tracer.end_span(validation_span_id)?;
    
    tracer.end_span(parent_span_id)?;
    
    let completed_spans = tracer.get_completed_spans();
    println!("✅ Created span hierarchy with {} spans:", completed_spans.len());
...
fn baggage_example() -> Result<(), tyl_tracing::TracingError> {
    ...
    println!("  - Correlation ID: {:?}", tracer.get_baggage("correlation_id"));
