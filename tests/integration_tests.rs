use tyl_tracing::{TracingManager, SimpleTracer, TraceConfig, Environment, TracingError};
...
fn test_end_to_end_tracing() {
    ...
    tracer.add_span_attribute(&span_id, "test_id", serde_json::json!("integration_001")).unwrap();
    tracer.add_span_attribute(&span_id, "environment", serde_json::json!("test")).unwrap();
...
fn test_span_lifecycle_integration() {
    ...
    let span3_id = tracer.start_span("operation_3", Some(span1_id.clone())).unwrap();
    
    // Add attributes
    tracer.add_span_attribute(&span1_id, "type", serde_json::json!("parent")).unwrap();
    tracer.add_span_attribute(&span3_id, "type", serde_json::json!("child")).unwrap();
    ...
    let child_span = completed_spans.iter()
...
fn test_baggage_context_integration() {
    ...
    if let Some(req_id) = tracer.get_baggage("request_id") {
        tracer.add_span_attribute(&span_id, "request_id", serde_json::json!(req_id)).unwrap();
    }
    
    if let Some(session) = tracer.get_baggage("user_session") {
        tracer.add_span_attribute(&span_id, "session", serde_json::json!(session)).unwrap();
    }
    
    tracer.end_span(span_id).unwrap();
    
    // Verify baggage retrieval
    assert_eq!(tracer.get_baggage("request_id"), Some("req_integration_123".to_string()));
    assert_eq!(tracer.get_baggage("user_session"), Some("session_abc".to_string()));
    assert_eq!(tracer.get_baggage("non_existent"), None);
    
    // Verify span has baggage-derived attributes
    let completed_spans = tracer.get_completed_spans();
    let span = &completed_spans[0];
    assert_eq!(span.attributes["request_id"], serde_json::json!("req_integration_123"));
...
fn test_error_handling_integration() {
    ...
    assert!(matches!(result.unwrap_err(), TracingError::InvalidSpanId { .. }));
    ...
    tracer.add_span_attribute(&span_id, "test", serde_json::json!("valid")).unwrap();
    tracer.end_span(span_id).unwrap();
...
fn test_max_spans_boundary_integration() {
    ...
    for i in 0..5 {
        ...
        tracer.add_span_attribute(&span_id, "iteration", serde_json::json!(i)).unwrap();
        tracer.end_span(span_id).unwrap();
    }
...
fn test_concurrent_access_safety() {
    ...
    for i in 0..5 {
        ...
        let handle = thread::spawn(move || {
            let span_id = tracer_clone.start_span(&format!("thread_operation_{}", i), None).unwrap();
            tracer_clone.add_span_attribute(&span_id, "thread_id", serde_json::json!(i)).unwrap();
